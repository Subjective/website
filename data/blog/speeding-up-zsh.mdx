...
draft: false
summary: In this post, I document the steps I took to speed up my Zsh startup times. I hope that anyone else struggling with sluggish startup times will find these optimizations helpful.
---
...
Recently, I've had a little bit too much time on my hands, so I finally decided to invest some effort into optimizing my setup. Needless to say, I'm quite happy with the results: I was able to slash Zsh's startup time down from 0.65 seconds to a _whopping_ 0.07 seconds!
...
There are several ways to lazy load nvm, such as [zsh-nvm](https://github.com/lukechilds/zsh-nvm), but I've personally found the [nvm](https://github.com/ohmyzsh/ohmyzsh/tree/master/plugins/nvm) plugin bundled with `Oh My Zsh` to be the fastest and easiest to configure. Just add the following lines to your `.zshrc` to enable lazy loading:
...
[Powerline10k](https://github.com/romkatv/powerlevel10k) is a popular prompt that is both performant and high-customizable. For our purposes, we are interested in its Instant Prompt feature, which displays the shell prompt before Zsh finishes starting up. While this doesn't actually eliminate startup lag, it gives the appearance of doing so by immediately showing the text for the shell prompt instead of waiting until after Zsh starts up. Personally, with the above optimizations, my shell startup is so fast that the difference is barely perceptible, but for other configurations, `Powerline10k` may improve the visual lag.
...
All in all, I was able to reduce my Zsh's startup time by roughly 90%. The table below summarizes my results in more detail:
...
| zim+fnm+zsh-defer                  | 0.17 s               |
| zim+fnm+evalcache+zsh-defer        | 0.07 s               |
...
- Deferring the execution of evals with the `Zsh-defer` plugin
